          ...
          <div className="collapse navbar-collapse me-5 pe-5" id="navbarSupportedContent">
            <ul className="navbar-nav ms-auto mb-2 mb-lg-0">
              <li className="nav-item dropdown">
                <i id="heart-fav-list" className="fas fa-chevron-down fs-3" role="button"
                  data-bs-toggle="dropdown"> Favoritos</i>
                {/* <a
                ...
                <ul className="dropdown-menu" style={{background: 'transparent', width:'80px'}}>
                  {favoritos.map(el => (
                    <li key={el.id} className='d-flex justify-content-between me-4 '>
                      <Link className="text-success titulo-info" style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '100%' }}to={el.mass ? `/people/${el.uid}` : el.model ? `/vehiculo/${el.uid}` : el.diameter ? `/planets/${el.uid}` : "" }>
                        {el.name}
                      </Link>
                      <a className='arrow' onClick={()=>deleteItem(el)}><i class="fas fa-trash-alt"></i></a>
                    </li>
                  ))}
                  <li>
